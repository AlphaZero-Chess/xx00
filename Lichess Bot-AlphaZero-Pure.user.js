...
const CONFIG = {
    // v29.0.0: TRUE ALPHAZERO - PARADIGM-SHIFTING ALIEN INTELLIGENCE
    thinkingTimeMin: 90000,         // 90 seconds minimum (DEEP STRATEGIC CALCULATION)
    thinkingTimeMax: 360000,        // 360 seconds maximum (6 MINUTES for complex positions!)
    premoveTime: 500,
    humanMistakeRate: 0.0,          // 0% (ABSOLUTE ZERO MISTAKES)
    
    // v29.0.0: MAXIMUM search depth - TRUE ALIEN INTELLIGENCE
    baseDepth: 64,                  // Base depth - BEYOND SUPERHUMAN (58→64)
    strategicDepth: 70,             // Strategic positions - DEEP WEB-WEAVING (62→70)
    endgameDepth: 76,               // Endgame - FLAWLESS TECHNIQUE (68→76)
    openingDepth: 58,               // Opening - PERFECT THEORY (52→58)
    classicalDepth: 76,             // Classical - MAXIMUM POWER (68→76)
    winningDepth: 68,               // Winning - CRUSHING CONVERSION (60→68)
    tacticalDepth: 72,              // Tactical - ABSOLUTE PRECISION (65→72)
    criticalDepth: 80,              // Critical - PARADIGM-SHIFT POWER (70→80)
    
    // v29.0.0: FORCING LINE DEPTH - 40+ MOVE TACTICAL HORIZON
    forcingLineDepth: 45,           // Depth for captures/checks (35→45)
    backRankMateDepth: 35,          // Back-rank patterns (25→35)
    queenInfiltrationDepth: 30,     // NEW: Queen invasion patterns
    passedPawnThreatDepth: 40,      // NEW: Passed pawn calculation
    
    // Time management - DEEP STRATEGIC CALCULATION
    earlyGameSpeed: 2.0,            // Maximum time in opening for theory
    middleGameSpeed: 3.0,           // MAXIMUM time in middlegame for webs
    endGameSpeed: 2.8,              // Extended endgame for perfect technique
    
    // v29.0.0: HOLISTIC POSITION EVALUATION - TRUE ALPHAZERO UNDERSTANDING
    positionWeight: 15.0,           // PARADIGM-SHIFT (10.0→15.0)
    initiativeBonus: 300,           // MASSIVE - tempo is everything (220→300)
    pieceActivityBonus: 350,        // CRITICAL - active pieces win (250→350)
    controlBonus: 250,              // HUGE - space control dominance (180→250)
    mobilityWeight: 12.0,           // MASSIVE - piece mobility (9.0→12.0)
    coordinationWeight: 15.0,       // CRITICAL - piece harmony (10.0→15.0)
    developmentWeight: 18.0,        // SEVERE punishment for undeveloped (12.0→18.0)
    centerControlWeight: 20.0,      // CRITICAL - central dominance (15.0→20.0)
    
    // v29.0.0: ABSOLUTE ZERO CREATIVITY - Perfect engine play with STRATEGIC DEPTH
    ...
    longTermFocus: 0.20,            // 20% long-term focus (0.12→0.20) - WEB-WEAVING
    eleganceThreshold: 0.00,        // ZERO elegant moves - ONLY BEST
    openingScoreDiffThreshold: 1,   // ABSOLUTE STRICT opening (2→1)
    pieceSafetyWeight: 18.0,        // PARAMOUNT piece safety (12.0→18.0)
    
    // Winning conversion - CRUSHING AND FLAWLESS
    winningThreshold: 120,          // Earlier winning detection (150→120)
    ...
    // v29.0.0: BACK-RANK MATE PREVENTION - CRITICAL ENHANCED SETTINGS
    backRankMateDetection: true,    // Enable back-rank mate detection
    backRankMatePenalty: 8000,      // EXTREME penalty for allowing back-rank mate (5000→8000)
    trapppedKingPenalty: 1200,      // HUGE penalty for king trapped on back rank (800→1200)
    openFileNearKingPenalty: 350,   // Penalty for open file near king (200→350)
    
    // v29.0.0: QUEEN INFILTRATION PREVENTION - NEW CRITICAL
    queenInfiltrationPenalty: 600,  // NEW: Penalty for allowing queen invasion
    queenOnSecondRankPenalty: 800,  // NEW: Enemy queen on 2nd/7th rank
    queenNearKingPenalty: 500,      // NEW: Enemy queen near our king
    
    // Tactical detection - BEYOND ABSOLUTE PERFECTION
    tacticalThreshold: 0.99,        // MAXIMUM threshold (0.95→0.99)
    threatResponseDepth: 20,        // MAXIMUM depth (15→20)
    forcingMoveBonus: 250,          // MAXIMUM bonus (180→250)
    evaluationDropThreshold: 15,    // INSTANT threshold (25→15)
    criticalEvalThreshold: -50,     // INSTANT threshold (-80→-50)
    
    // Anti-draw and repetition
    repetitionPenalty: 200,         // EXTREME penalty (150→200)
    antiDrawBias: 1.00,             // ABSOLUTE bias against draws (0.99→1.00)
    repetitionHistoryDepth: 20,     // Track more positions (15→20)
    
    // DEFENSIVE MODE - ABSOLUTE PERFECTION
    defensiveThresholdMild: -30,    // INSTANT (-40→-30)
    defensiveThresholdSerious: -60, // INSTANT (-80→-60)
    defensiveThresholdCritical: -120, // INSTANT (-150→-120)
    ...
    defensiveDepthBonus: 16,        // MAXIMUM extra depth (+12→+16)
    
    // v29.0.0: SACRIFICE VALIDATION SYSTEM (ENHANCED)
    sacrificeMinCompensation: 350,  // Higher compensation required (200→350)
    sacrificeValidationDepth: 25,   // Deeper verification (15→25)
    materialCrisisThreshold: -50,   // Earlier crisis detection (-80→-50)
    
    // v29.0.0: COUNTERPLAY GENERATION (ENHANCED)
    counterplayPriority: 400,       // HUGE bonus for threats when behind (250→400)
    passivePlayPenalty: -500,       // SEVERE penalty for passive moves (-300→-500)
    initiativePremiumWhenBehind: 3.0, // MASSIVE multiplier (2.0→3.0)
    
    // v29.0.0: QUEEN TRADE EVALUATION (ENHANCED)
    queenTradeWhenBehindPenalty: -600, // EXTREME penalty (-400→-600)
    queenTradeWhenAheadBonus: 200,  // Good bonus when ahead (150→200)
    
    // v29.0.0: BISHOP PAIR PRESERVATION (ENHANCED)
    bishopPairValue: 180,           // Higher value (120→180)
    bishopTradeInClosedPenalty: -250, // Stronger penalty (-150→-250)
    
    // Passed pawn settings - CRITICAL FOR ENDGAME
    passedPawnDangerRank: 1,
    passedPawnDepthBonus: 8,        // Much higher (0→8)
    
    // v29.0.0: ENHANCED POSITIONAL WEIGHTS - TRUE ALPHAZERO UNDERSTANDING
    pawnStructureWeight: 35.0,      // CRITICAL foundation (25.0→35.0)
    kingSafetyWeight: 70.0,         // PARAMOUNT PRIORITY (50.0→70.0)
    weakSquareWeight: 30.0,         // Strong exploitation (20.0→30.0)
    passedPawnWeight: 35.0,         // CRITICAL value (22.0→35.0)
    pieceHarmonyWeight: 28.0,       // Piece coordination (18.0→28.0)
    spaceAdvantageWeight: 24.0,     // Space control (16.0→24.0)
    
    // v29.0.0: SUPREME WEIGHTS - PARADIGM SHIFTING
    tempoWeight: 45.0,              // TEMPO IS EVERYTHING (30.0→45.0)
    developmentUrgency: 50.0,       // SEVERE undeveloped penalty (35.0→50.0)
    tacticalHorizon: 45,            // 45 ply for forcing lines (30→45)
    weakeningPawnPenalty: 400,      // g3/h3 near king DANGEROUS (250→400)
    consecutiveWastedMovesPenalty: 350, // b3→a3→a4 patterns (200→350)
    exchangeSacrificeCaution: 600,  // Exchange sacs need BIG compensation (400→600)
    
    // v29.0.0: 50+ MOVE STRATEGIC PLANNING - TRUE WEB-WEAVING
    longTermPlanningDepth: 50,      // Evaluate positions 50+ moves ahead (40→50)
    rolloutSimulations: 20,         // More simulations (10→20)
    rolloutDepthPerSim: 18,         // Deeper per simulation (12→18)
    strategicWebWeight: 35.0,       // Weight for strategic plans (20.0→35.0)
    prophylaxisWeight: 25.0,        // Preventing opponent plans (15.0→25.0)
    
    // v29.0.0: MCTS-INSPIRED SIMULATION (PARADIGM-SHIFT)
    mctsSimulations: 30,            // More Monte Carlo simulations (15→30)
    mctsDepthPerSim: 25,            // Much deeper per simulation (15→25)
    mctsStrategicWeight: 40.0,      // MCTS evaluation weight (25.0→40.0)
    
    // v29.0.0: PATTERN RECOGNITION - ALIEN INTELLIGENCE
    discoveredAttackBonus: 900,     // HUGE reward for discoveries (600→900)
    discoveredAttackPenalty: -1000, // SEVERE penalty for missing (-700→-1000)
    matingPatternDepth: 30,         // Deep mating patterns (20→30)
    
    // Castling bonus - KING SAFETY IS PARAMOUNT
    castlingBonus: 500,             // MASSIVE bonus (350→500)
    kingInCenterPenalty: 400,       // SEVERE penalty after move 8 (250→400)
...
/**
 * v29.0.0: TRUE MCTS - Monte Carlo Tree Search with UCB1 Selection
 * Implements genuine tree search for 40+ move strategic webs
 */
function trueMCTSEvaluate(fen, candidateMove, depth = 50) {
    let totalScore = 0;
    let wins = 0;
    let visits = 0;
    ...
    try {
        ...
        for (let sim = 0; sim < CONFIG.mctsSimulations; sim++) {
            ...
            let simulationPath = [candidateMove];
            
            // Expansion and Simulation combined
            const rolloutResult = deepStrategicRollout(fen, candidateMove, CONFIG.mctsDepthPerSim, sim);
            
            // Backpropagation
            rootNode.visits++;
            rootNode.totalValue += rolloutResult.score;
            
            if (rolloutResult.isWinning) wins++;
            visits++;
            
            totalScore += rolloutResult.score;
        }
        
        if (visits === 0) return 0;
        
        // Calculate final MCTS score with UCB1 exploration bonus
        const avgScore = totalScore / visits;
        const winRate = wins / visits;
        const explorationBonus = Math.sqrt(2 * Math.log(visits + 1) / (rootNode.visits + 1));
        
        // Weighted combination of average score, win rate, and exploration
        const mctsScore = avgScore * 0.6 + 
                          (winRate * 500) * 0.3 + 
                          (explorationBonus * 50) * 0.1;
        
        debugLog("[MCTS_TRUE]", `🎯 Move ${candidateMove}: visits=${visits}, wins=${wins}, avgScore=${avgScore.toFixed(1)}, winRate=${(winRate*100).toFixed(1)}%, final=${mctsScore.toFixed(1)}`);
        
        return mctsScore;
    } catch (e) {
        debugLog("[MCTS_TRUE]", `⚠️ Error: ${e.message}`);
        return 0;
    }
}
